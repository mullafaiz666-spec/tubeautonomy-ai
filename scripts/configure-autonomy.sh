...
echo "TubeAutonomy one-time secure YouTube setup"
echo "TubeAutonomy now works without AI/trend API keys: yt-dlp handles public trend metadata and Qwen3 writes locally."
echo "Optional keys can improve capability, but only Google OAuth is required for unattended YouTube upload."
echo "Secrets are sent directly to GitHub and are never written to this repository."

read -rsp "Optional YouTube Data API key [press Enter to use yt-dlp]: " YOUTUBE_API_KEY; echo
read -rsp "Optional Gemini API key [press Enter to use local Qwen3]: " GEMINI_API_KEY; echo
read -rsp "Google OAuth client ID (required for upload): " YOUTUBE_CLIENT_ID; echo
read -rsp "Google OAuth client secret (required for upload): " YOUTUBE_CLIENT_SECRET; echo

for value_name in YOUTUBE_CLIENT_ID YOUTUBE_CLIENT_SECRET; do
...
if [ -n "$YOUTUBE_API_KEY" ]; then
  ...
  echo "YouTube Data API key saved; official discovery will be preferred."
else
  gh secret delete YOUTUBE_API_KEY --repo "$REPO" >/dev/null 2>&1 || true
  echo "No YouTube Data API key: yt-dlp public metadata discovery will be used."
fi
if [ -n "$GEMINI_API_KEY" ]; then
  printf '%s' "$GEMINI_API_KEY" | gh secret set GEMINI_API_KEY --repo "$REPO"
  echo "Gemini key saved; Gemini will be preferred for writing."
else
  gh secret delete GEMINI_API_KEY --repo "$REPO" >/dev/null 2>&1 || true
  echo "No Gemini key: local Qwen3 writer will be used."
fi
...
read -rp "Publishing privacy private/unlisted/public [private]: " YOUTUBE_PRIVACY
YOUTUBE_PRIVACY="${YOUTUBE_PRIVACY:-private}"
...
print(json.dumps({
  ...
  'format': sys.argv[4],
  'privacy': sys.argv[5],
  'writerFallback': 'Qwen/Qwen3-0.6B',
  'trendFallback': 'yt-dlp',
}))
PY
)"
...
echo "YouTube authorization configured. Triggering the first autonomous content run on the isolated branch..."
